  ...
  return (
    <div>
      <button
        ...
        className="flex w-full items-center gap-1.5 py-0.5 text-left hover:text-white text-slate-300 transition-colors"
        style={{ paddingLeft: `${depth * 14}px` }}
      >
        <span className="text-slate-500 w-4 font-mono">{open ? '▾' : '▸'}</span>
      ...
      {open && [...node.children.values()].map((c, i) => (
        c.children.size > 0 ? (
          ...
          <div
            key={c.name + i}
            className="flex items-center gap-1.5 py-0.5 text-slate-400 hover:text-slate-200"
            style={{ paddingLeft: `${(depth + 1) * 14}px` }}
          >
            <span className="text-slate-600 w-4 font-mono">·</span>
            <span className="truncate">{c.name}</span>
            <span className="text-[11px] text-slate-500 ml-auto pl-3 font-mono">{formatBytes(c.size)}</span>
          </div>
        )
      ))}
    </div>
...
export default function FileTree({ files }) {
  ...
  return (
    <div className="rounded-lg border border-ink-800 bg-ink-950 px-3 py-2.5 max-h-72 overflow-y-auto font-mono text-xs">
      {[...root.children.values()].map((c, i) =>
        c.children.size > 0 ? (
          <Dir key={c.name + i} node={c} depth={0} />
        ) : (
          <div key={c.name + i} className="flex items-center gap-1.5 py-0.5 text-slate-400 hover:text-slate-200" style={{ paddingLeft: '14px' }}>
            <span className="text-slate-600 w-4 font-mono">·</span>
            <span className="truncate">{c.name}</span>
            <span className="text-[11px] text-slate-500 ml-auto pl-3 font-mono">{formatBytes(c.size)}</span>
          </div>
