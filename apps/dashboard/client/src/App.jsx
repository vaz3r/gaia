...
function Stat({ label, value, sub, accent }) {
  return (
    <div className="rounded-lg bg-ink-800 px-4 py-3 border border-ink-700">
      ...
      <div className={`text-2xl font-semibold mt-1 ${accent ? 'text-accent' : 'text-slate-100'}`}>{value}</div>
      ...
      <header className="border-b border-ink-700 bg-ink-900/60 sticky top-0 z-10 backdrop-blur">
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-xs text-slate-400">
            ...
            {stats?.session_uptime_s ? ` · up ${formatUptime(stats.session_uptime_s)}` : ''}
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 py-6">
        <section className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-8">
          <Stat
            ...
            value={formatNum(stats?.total_torrents)}
            sub={`${formatNum(stats?.verified_last_24h)} / 24h`}
          />
          <Stat
            label="Verified 1h"
            value={formatNum(stats?.verified_last_1h)}
            accent
          />
          <Stat label="Unique infohashes" value={formatNum(stats?.seen_last_1h)} sub={`${formatNum(stats?.new_last_1h)} new`} />
          <Stat
            label="Queue"
            value={formatNum(stats?.queue_backlog)}
            sub={`${formatNum(stats?.verifying)} verifying`}
          />
          <Stat label="Crawler" value={stale === null ? '—' : stale > 180 ? 'stale' : 'online'} />
          <Stat label="Session" value={formatUptime(stats?.session_uptime_s)} />
        </section>
