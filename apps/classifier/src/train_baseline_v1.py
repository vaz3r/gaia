#!/usr/bin/env python3
"""
Reproducible FP32 MiniLM Baseline Training and Evaluation Script (Baseline v1).
Trains sentence-transformers/all-MiniLM-L12-v2 on clean baseline_v1 splits,
selects the best checkpoint via validation macro F1, and evaluates on the frozen
Gold Pilot v1 reference benchmark across full, sparse, and rich subsets.
"""
...
logger = logging.getLogger("train_baseline_v1")
...
EXPECTED_CHECKSUMS = {
    "train.jsonl": "8c7ead7c3b901cab18c9d593e6b6a905246918753d441140da626c8221398a42",
...
def verify_and_load_data(
    ...
    """Verify checksums, schemas, and prepare tokenized datasets."""
    # 1. Checksums
    train_sha = calc_sha256(train_path)
    val_sha = calc_sha256(val_path)
    ref_sha = calc_sha256(ref_path)

    if train_sha != EXPECTED_CHECKSUMS["train.jsonl"]:
        raise ValueError(f"Train checksum mismatch: {train_sha} != {EXPECTED_CHECKSUMS['train.jsonl']}")
    ...
    if len(train_rows) != 3553:
        raise ValueError(f"Expected 3553 train rows, got {len(train_rows)}")
    if len(val_rows) != 627:
    ...
    # 3. Tokenize texts
    def process_data(rows: list[dict], is_ref: bool = False):
    ...
    ref_data = process_data(ref_rows, is_ref=True)
...
def compute_metrics(
    ...
    """Compute detailed evaluation metrics with explicit fixed 8-class ordering."""
...
def evaluate(
    ...
    """Run inference over dataloader and return loss, predictions, confidences, top-3 candidates."""
    model.eval()
    total_loss = 0.0
    total_weight = 0.0
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            logits = outputs.logits  # [batch_size, 8]
            ...
            probs = F.softmax(logits, dim=-1)  # [batch_size, 8]
...
def train_and_evaluate(
    train_path: str = "apps/classifier/data/baseline_v1/train.jsonl",
    val_path: str = "apps/classifier/data/baseline_v1/validation.jsonl",
    ref_path: str = "apps/classifier/data/gold_pilot_v1/reference_eval_v1.jsonl",
    out_dir: str = "apps/classifier/data/models/transformer/baseline_v1",
    ...
    # Compute balanced class weights on 3,553 train rows, clipped to [0.5, 3.0]
    ...
    logger.info(f"Clipped class weights: {class_weights_dict}")
    ...
    # Record model architecture parameters
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        # Checkpoint selection: highest val macro F1; tie-breaker: lower val loss, earlier epoch
        is_best = False
    ...
    # 2. Reference evaluation (Run exactly once on finalized best checkpoint)
    ...
    # Run manifest
    manifest = {
        "run_name": "baseline-v1-minilm-l12-fp32",
        ...
        "known_limitations": [
            "training and validation labels are predominantly heuristic",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train and evaluate FP32 MiniLM baseline.")
    parser.add_argument("--train", default="apps/classifier/data/baseline_v1/train.jsonl")
    parser.add_argument("--val", default="apps/classifier/data/baseline_v1/validation.jsonl")
    parser.add_argument("--ref", default="apps/classifier/data/gold_pilot_v1/reference_eval_v1.jsonl")
    parser.add_argument("--out_dir", default="apps/classifier/data/models/transformer/baseline_v1")
