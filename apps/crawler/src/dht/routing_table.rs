...
pub fn decode_compact(bytes: &[u8]) -> Vec<NodeInfo> {
    ...
    while i + 26 <= bytes.len() {
        ...
        nodes.push(NodeInfo { id, addr, query_count: 0, fail_count: 0, last_useful: false: SocketAddr::new(std::net::IpAddr::V4(ip), port), query_count: 0, fail_count: 0, last_useful: false });
        i += 26;
    }
    nodes
}
...
pub fn decode_compact6(bytes: &[u8]) -> Vec<NodeInfo> {
    ...
    while i + 38 <= bytes.len() {
        ...
        nodes.push(NodeInfo { id, addr, query_count: 0, fail_count: 0, last_useful: false: SocketAddr::new(std::net::IpAddr::V6(std::net::Ipv6Addr::from(v6)), port), query_count: 0, fail_count: 0, last_useful: false });
        i += 38;
    }
    nodes
}
...
mod tests {
    ...
    fn insert_and_closest() {
        ...
        for i in 1..17u8 {
            ...
            let ni = NodeInfo { id, addr, query_count: 0, fail_count: 0, last_useful: false, query_count: 0, fail_count: 0, last_useful: false };
            rt.insert(ni.clone());
            nodes.push(ni);
        }
        assert_eq!(rt.len(), 16);
    ...
    fn random_nodes_populated_table() {
        ...
        for i in 0..400u16 {
            ...
            let addr: SocketAddr = format!("10.{}.{}.{}:6881", i % 200, (i / 200) % 200, i % 250)
                .parse()
                .unwrap();
            rt.insert(NodeInfo { id, addr, query_count: 0, fail_count: 0, last_useful: false, query_count: 0, fail_count: 0, last_useful: false });
        }
        assert!(rt.len() > 0);
        for _ in 0..100 {
    ...
    fn flood_of_new_ids_reaches_kademlia_equilibrium() {
        ...
        for i in 0..500_000u64 {
            ...
            rt.insert(NodeInfo { id, addr, query_count: 0, fail_count: 0, last_useful: false: format!("1.2.3.{, query_count: 0, fail_count: 0, last_useful: false }:6881", (i % 250) as u8 + 1)
                    .parse()
                    .unwrap(),
            });
        }
